...
const mutation = useMutation({
  ...
  onError: (error: any) => {
    if (error.message === 'Validation failed' && error.issues) {
      ...
      for (const [field, messages] of Object.entries(error.issues)) {
        mapped[field as keyof typeof errors] =
  ...
  // app/signup/page.tsx - Replace the handleSubmit function
        ...
        <div className="relative z-10 flex w-full flex-col bg-black text-white md:w-1/2 md:min-h-screen">
          ...
          <div className="flex flex-1 flex-col items-center justify-center  sm:gap-4 sm:px-6 sm:py-8 md:gap-5 md:px-8">
            <div className="text-center max-sm:px-2">
              <h1 className="text-xl font-bold sm:text-2xl md:text-3xl">Create account</h1>
              <p className="mt-1 text-[10px] text-gray-300 sm:mt-2 sm:text-xs md:text-sm">Join us today! It&apos;s free and easy.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex w-full max-w-[260px] flex-col gap-3 sm:max-w-[280px] sm:gap-4 md:max-w-[300px]">
              <div className="flex flex-col gap-1 sm:gap-1.5">
                <label className="text-[10px] font-medium sm:text-xs md:text-sm">Full Name</label>
                ...
                {errors.name && <p className="mt-1 text-[9px] text-red-400 sm:text-[10px] md:text-xs">{errors.name}</p>}
              </div>
              ...
              <div className="flex flex-col gap-1 sm:gap-1.5">
                ...
                {errors.email && <p className="mt-1 text-[9px] text-red-400 sm:text-[10px] md:text-xs">{errors.email}</p>}
              </div>
              ...
              <div className="flex flex-col gap-1 sm:gap-1.5">
                ...
                {errors.password && <p className="mt-1 text-[9px] text-red-400 sm:text-[10px] md:text-xs">{errors.password}</p>}
              </div>

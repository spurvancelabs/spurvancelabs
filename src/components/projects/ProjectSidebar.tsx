...
const icons = {
  ...
  activity: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
  ...
  back: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M10.5 19.5h3m-6.75 2.25h10.5a2.25 2.25 0 002.25-2.25v-2.25m-8.25 2.25V9m0 0l-3 3m3-3l3 3M3.75 12.75V5.25A2.25 2.25 0 016 3h12a2.25 2.25 0 012.25 2.25v7.5" /></svg>,
...
export default function ProjectSidebar({ project }: { project?: ProjectInfo }) {
  ...
  const navItems = [
    ...
    ...(projectId ? [
      ...
      { href: `/projects/${projectId}/activity`, label: 'Activity', icon: 'activity' as const },
    ...
    <aside className="fixed left-0 top-0 h-screen w-64 bg-zinc-950 border-r border-white/[0.06] flex flex-col z-40">
      ...
      <div className="p-3 border-t border-white/[0.06] space-y-1">
        {userRole && isAdminRole(userRole) && (
          ...
          >
            <span className="shrink-0">{icons.back}</span>
            Back to Admin
          </Link>
        )}
        <button

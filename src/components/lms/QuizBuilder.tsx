    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={openSettings} className="text-gray-400 hover:text-white transition-colors" title="Quiz settings">
          ...
          <label className="text-gray-400 hover:text-white transition-colors cursor-pointer" title="Import from CSV">
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
            </svg>
            <input type="file" accept=".csv" onChange={handleCsvImport} className="hidden" />
          </label>
          <button onClick={() => { if (window.confirm('Delete this quiz and all its questions?')) deleteQuiz.mutate() }} disabled={deleteQuiz.isPending} className="text-gray-400 hover:text-red-400 transition-colors" title="Delete quiz">
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {showSettings && (
        <div className="border border-white/[0.06] rounded-lg p-3 space-y-2.5 bg-zinc-900/40">
          <div className="grid grid-cols-2 gap-2.5">
            ...
            <div>
              <label className="block text-[11px] text-gray-400 mb-1">Passing Score (%)</label>
            ...
            <div>
              <label className="block text-[11px] text-gray-400 mb-1">Time Limit (min)</label>
              <input type="number" min="0" value={settingsForm.timeLimit} onChange={e => setSettingsForm(f => ({ ...f, timeLimit: e.target.value }))}
                className={`w-full bg-zinc-800 border border-white/[0.08] rounded-lg px-3 py-1.5 text-white text-sm focus:outline-none ${variant === 'admin' ? 'focus:border-blue-500/50' : 'focus:border-amber-500/50'}`} />
            </div>
            <div>
              <label className="block text-[11px] text-gray-400 mb-1">Max Attempts</label>
            ...
            <div>
              <label className="block text-[11px] text-gray-400 mb-1">Shuffle Questions</label>
              <div className="flex items-center h-[34px]">
                ...
                <span className="ml-2 text-xs text-gray-400">{settingsForm.shuffleQuestions ? 'Yes' : 'No'}</span>
              </div>
            </div>
            <div>
              <label className="block text-[11px] text-gray-400 mb-1">Show Results</label>
              <div className="flex items-center h-[34px]">
                ...
                <span className="ml-2 text-xs text-gray-400">{settingsForm.showResults ? 'Yes' : 'No'}</span>
              </div>
            </div>
          </div>
            ...
            <div key={q.id}>
              <div className="flex items-start gap-2 px-2 py-1.5 rounded-lg hover:bg-white/[0.02] group">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-gray-400 mt-0.5">{q.points} pt{q.points !== 1 ? 's' : ''}</p>
                </div>
                <button onClick={() => openEditQuestion(q)} className="text-gray-400 hover:text-white transition-colors opacity-0 group-hover:opacity-100" title="Edit question">
                  <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                  </svg>
                </button>
                <button onClick={() => { if (window.confirm('Delete this question?')) deleteQuestion.mutate(q.id) }} className="text-gray-400 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100" title="Delete question">
                  <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {quiz.questions && quiz.questions.length === 0 && !addingQuestion && (
        <p className="text-[11px] text-gray-400">No questions yet. Add one or import from CSV.</p>
      )}
    </div>
  )
}
  ...
  return (
    <div className="border border-white/[0.06] rounded-lg p-3 space-y-2.5 bg-zinc-900/40 mt-1">
      <div className="grid grid-cols-2 gap-2.5">
        ...
        <div>
          <label className="block text-[11px] text-gray-400 mb-1">Points</label>
      ...
      <div>
        <label className="block text-[11px] text-gray-400 mb-1">Question</label>
      ...
      {form.type === 'multiple_choice' && (
        <div>
          <label className="block text-[11px] text-gray-400 mb-1">Options</label>
          <div className="space-y-1.5">
            {form.options.map((opt, i) => (
              <div key={i} className="flex gap-1.5 items-center">
                ...
                {form.options.length > 2 && (
                  <button onClick={() => removeOpt(i)} className="text-gray-400 hover:text-red-400 transition-colors shrink-0">
                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                    </svg>
                  </button>
          ...
          <button onClick={addOpt} className="text-[11px] text-gray-400 hover:text-gray-300 mt-1">+ Add option</button>
        </div>
      )}

      {form.type === 'true_false' && (
        <div>
          <label className="block text-[11px] text-gray-400 mb-1">Correct Answer</label>
          <div className="flex gap-2">
      ...
      {(form.type === 'short_answer' || form.type === 'essay') && (
        <div>
          <label className="block text-[11px] text-gray-400 mb-1">Correct Answer</label>
          {form.type === 'essay' ? (
            <p className="text-[11px] text-gray-400">Essay questions are graded manually.</p>
          ) : (
